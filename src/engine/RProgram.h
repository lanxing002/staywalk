...
namespace staywalk{
	...
	class sw_Class(jsonpost;)  StdProgram : public RObject {
	public:
	...
	class sw_Class(jsonpost;)  CSProgram : public RObject {
	public:
...
namespace staywalk {
	template<>
	inline void StdProgram::set_uniform<bool>(const string& name, bool value) {
		glUniform1i(get_uniform(name), value);
	}

	template<>
	inline void StdProgram::set_uniform<int>(const string& name, int value) {
		glUniform1i(get_uniform(name), value);
	}

	template<>
	inline void StdProgram::set_uniform<float>(const string& name, float value) {
		glUniform1f(get_uniform(name), value);
	}

	template<>
	inline void StdProgram::set_uniform<vec2>(const string& name, vec2 value) {
		glUniform2fv(get_uniform(name), 1, &value[0]);
	}

	template<>
	inline void StdProgram::set_uniform<vec3>(const string& name, vec3 value) {
		glUniform3fv(get_uniform(name), 1, &value[0]);
	}

	template<>
	inline void StdProgram::set_uniform<vec4>(const string& name, vec4 value) {
		glUniform4fv(get_uniform(name), 1, &value[0]);
	}

	template<>
	inline void StdProgram::set_uniform<mat4>(const string& name, mat4 value) {
